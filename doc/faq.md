...
这是 `dsh-file-mount` 往会话里写了新内核不认的字段。该插件已从默认包撤下。在网页「设置 → 插件」里卸载它，或从 `home\profiles\web\package.json` 的 `dependencies` / `bundles` 去掉 `dsh-file-mount` 后重启。对话文件仍在 `home/sessions`。
