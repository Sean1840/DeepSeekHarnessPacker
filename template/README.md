...
- **不用重新下载压缩包、不用手动替换文件。** 双击 `update.cmd` 会：
  1. 从 GitHub Release 下载最新便携包，原地替换程序文件（脚本、dsh、内置插件）；
  2. 再检查 dsh 引擎是否还有更新；
  3. **`home` 目录不覆盖**（对话、API Key、个人设置都在），你改过的 `config.json` 项也会保留。
...
**Q：离线机器如何更新？**
在能联网的机器上双击 `update.cmd` 完成更新，再把整个文件夹拷过去。不要只拷 `home`，也不要覆盖正在用的 `home`。
