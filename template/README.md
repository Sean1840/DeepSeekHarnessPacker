...
| [更新](doc/update.md) | 在线 / 离线更新；老包如何打补丁；内核与基础插件都需确认 |
| [配置](doc/config.md) | `config.json` |
| [插件](doc/plugins.md) | 本包自带的基础插件 |
| [常见问题](doc/faq.md) | 端口、离线、数据在哪 |

更新时：**不自动替换你已装的插件**；**dsh 内核要你同意才换**；内核换了且基础插件落后时会再问要不要刷新（自己装的插件仍保留）。`home`（对话、设置）保留。老版本便携包请下 GitHub Release 里的 `DeepSeekHarness-updater-v*.zip`（不用解完整包），见 [给老包打补丁](doc/update.md#给老包打补丁v13-及更早)。
