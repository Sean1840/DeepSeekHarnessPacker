...
- **不用手动替换文件夹。** 双击 `update.cmd` 会：
  ...
  4. **不会替换你已经在用的插件**（压缩包只提供首次安装的基础插件）。若某些插件比本包旧，只会提示，由你在网页里自行升级；
  5. **dsh 内核**若有新版本会询问，只有你同意才替换；
  6. **`home` 目录不覆盖**（对话、API Key、个人设置都在），你改过的 `config.json` 项也会保留。
...
不会。`home` 目录（含你装的插件）和你改过的 `config.json` 都保留。dsh 内核只有你在提示里同意才会替换。
